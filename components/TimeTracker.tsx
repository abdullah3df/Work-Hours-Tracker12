...
const TimeTracker: React.FC<TimeTrackerProps> = ({ addLog, profile, t, showToast }) => {
  ...
  return (
    <div className="bg-glass-bg-light dark:bg-glass-bg-dark backdrop-blur-xl border border-glass-border-light dark:border-glass-border-dark p-6 sm:p-8 rounded-2xl shadow-2xl shadow-shadow-color-light dark:shadow-shadow-color-dark w-full max-w-4xl mx-auto transition-all duration-300">
      ...
      <div className="flex justify-center items-center my-8 gap-4 sm:gap-8">
        {/* Clock In Button */}
        <button
          onClick={handleStart}
          disabled={isRunning}
        ...
        >
          <ShiftStartIcon className="w-12 h-12 md:w-14 md:h-14" />
          <span className="text-xl md:text-2xl font-semibold mt-2">{t('clockIn')}</span>
        </button>
        ...
        >
          <ShiftEndIcon className="w-12 h-12 md:w-14 md:h-14" />
          <span className="text-xl md:text-2xl font-semibold mt-2">{t('clockOut')}</span>
        </button>
      </div>
