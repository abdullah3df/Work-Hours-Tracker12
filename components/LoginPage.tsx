...
const LoginPage: React.FC<LoginPageProps> = ({ language, setLanguage, theme, setTheme, t, firebaseInitialized, onGuestLogin }) => {
    ...
    const handleGoogleSignIn = () => {
        if (!firebaseInitialized) {
            alert("Firebase is not configured. Please add your Firebase project configuration to index.html");
            return;
        }
        setError(null);
    ...
    return (
        <div className="min-h-screen flex flex-col items-center justify-center p-4">
            <header className="absolute top-0 inset-x-0 p-4 z-10">
                <div className="container mx-auto flex justify-end items-center space-x-2 sm:space-x-4">
                    <div className="relative" ref={languageMenuRef}>
                        ...
                        {isLanguageMenuOpen && (
                            <div className="absolute end-0 mt-2 w-40 bg-white/50 dark:bg-gray-800/50 backdrop-blur-lg rounded-md shadow-lg ring-1 ring-black ring-opacity-5 py-1 z-50">
                            {languageOptions.map((lang) => (
                                ...
                                className="flex justify-between items-center px-4 py-2 text-sm text-gray-700 hover:bg-white/50 dark:text-gray-200 dark:hover:bg-gray-700/50"
                                >
            ...
            <main className="w-full max-w-sm bg-glass-bg-light dark:bg-glass-bg-dark backdrop-blur-xl border border-glass-border-light dark:border-glass-border-dark rounded-2xl shadow-2xl shadow-shadow-color-light dark:shadow-shadow-color-dark p-8 text-center">
                ...
                {!firebaseInitialized && !error && (
                    <p className="text-xs text-red-500 mt-4">
                        Firebase not configured. Please check console.
                    </p>
                )}
            </main>
        </div>
